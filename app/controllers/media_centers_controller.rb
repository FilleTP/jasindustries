class MediaCentersController < ApplicationController
  ...
  def create
    ...
    unless @media_center.photos.empty?
      ...
      render :new, notice: "You must upload at least 1 image"
    end
  end
  ...
  def show
    ...
    @photos = @media_center.photos.drop(1)
  end
  ...
  def media_center_params
    params.require(:media_center).permit(:title, :content, photos: [])
  end
end
